...
// Instantiate UserManager and expose globally
...
function renderEverywhere() {
  ...
  // FILE UPLOADER
  $(document)
    .off('change.genericFileUploader')
    .on('change.genericFileUploader', '.fileUploader', function (event) {
      // image upload
    ...
    if (Meteor.isCordova) {
      ...
      window.open = /** @type {any} */ ((url) => {
        ...
        if (Meteor.isCordova && cordova.platformId === 'ios') {
          ...
          const win = originalWindowOpen(url, '_blank');
          setTimeout(() => {
            isOpening = false;
          }, 1000);
          return win;
...
Template.registerHelper('myChambers', function () {
  ...
  // Reactively fetch data from UserManager
  const chambers = (userManager.getData().chamberFollows || []).slice(); // `getData()` is reactive
