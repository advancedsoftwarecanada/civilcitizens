...
const DRIVE_NAV_ITEMS = [
  { href: '/drive', label: 'My Rides', icon: HiOutlineClock, aliases: ['/drive/ride/request'], driverOnly: false },
...
export default function DriveRouteNav() {
  ...
  const { isDriverActive } = useDriveViewerState()
  const items = DRIVE_NAV_ITEMS.filter((item) => isDriverActive || !item.driverOnly)

  return (
    <>
      <DriveActiveRideLocationSync enabled />
      <nav className="w-full" aria-label="Drive sections">
        <div className={`grid w-full gap-3 ${items.length >= 4 ? 'grid-cols-2 xl:grid-cols-4' : 'grid-cols-2'}`}>
          {items.map((item) => {
