...
function formatSavedAddressTitle(address: SavedShippingAddress) {
  return address.label?.trim() || address.name?.trim() || address.line1?.trim() || 'Saved address'
}

export default function ShippingAddressesPanel({
  title = 'Shipping Addresses',
  basePath = '/settings/addresses',
}: {
  title?: string
  basePath?: string
}) {
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="text-xs font-semibold text-slate-900">{formatSavedAddressTitle(item)}</p>
                      {item.isDefault ? (
