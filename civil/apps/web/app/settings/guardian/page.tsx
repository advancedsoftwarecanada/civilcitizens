    ...
    try {
      ...
      if (!response.ok || !payload) {
        if (payload?.error === 'family_mode_not_available') {
          pushToast('Guardian Mode is not available yet on this server. Apply the Guardian Mode database migration first.', 'error')
        } else {
          pushToast('Unable to load Guardian Mode right now.', 'error')
        }
        return
      }
    ...
    try {
      ...
      if (!response.ok) {
        if (payload?.error === 'family_profile_incomplete') {
          pushToast('Complete your profile before enabling Guardian Mode.', 'error')
        } else {
          pushToast('Unable to enable Guardian Mode right now.', 'error')
        }
        return
      }

      pushToast('Guardian Mode enabled.', 'success')
      setProfileTruthConfirmed(false)
      setChildSafetyAccepted(false)
      await loadFamilyData()
    } catch (error) {
      console.error('Failed to enable guardian mode', error)
      pushToast('Unable to enable Guardian Mode right now.', 'error')
    } finally {
      setSaving(false)
    }
  ...
  return (
    <DashboardShell rightRail={<RightRail />}>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-1 text-3xl font-semibold text-slate-950">Guardian Mode</h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">
                Guardian Mode lets a parent or guardian create supervised profiles for children, adolescents, and youth under the main account.
                New profiles now start in a dedicated draft editor before they appear in the supervised family list.
              </p>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-amber-900/80">
                  Guardian Mode requires a real parent or guardian identity on file. Please complete the missing profile details below in
                  your profile editor before enabling Guardian Mode.
                </p>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm leading-6 text-slate-600">
                      Before Guardian Mode can be enabled, the parent or guardian must attest that their identity details are true and that
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4 text-sm text-slate-700">
                    <p className="font-semibold text-slate-900">Guardian Mode status</p>
                  ...
                  {!familyData.familyMode.enabled ? (
                    <>
                      ...
                      <label className="flex items-start gap-3 rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-700">
                        ...
                        />
                        <span>
                          I understand that Guardian Mode is for supervised child, adolescent, and youth accounts and I have reviewed Civil&apos;s child safety guidance.
                        </span>
                      </label>
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        >
                          {saving ? 'Enabling…' : 'Enable Guardian Mode'}
                        </button>
                        <button
                          type="button"
                    ...
                    <div className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-700">
                      ...
                      Guardian Mode is enabled for this parent account.
                    </div>
                  )}
                </div>
              </div>
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              {!familyData.familyMode.enabled ? (
                <p className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-500">
                  Enable Guardian Mode above before creating supervised profiles.
                </p>
              ) : null}
                  ...
                  <article key={member.id} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
                    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_15rem]">
                      <div className="min-w-0">
                        ...
                        <p className="mt-3 text-xs font-semibold uppercase tracking-wide text-slate-400">Username</p>
                        <p className="mt-1 font-mono text-sm text-slate-700">{member.username}</p>
                        {member.suspended ? (
                          <div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-900">
                            ...
                            <p className="mt-1">{member.suspensionNote ?? 'Your account has been suspended by Guardian Mode, please ask your parent or guardian to restore your account.'}</p>
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                        ...
                        <div className="mt-3 flex flex-col gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() => router.push(`/settings/guardian/edit?id=${encodeURIComponent(member.id)}`)}
      ...
      >
        <div className="space-y-5 text-sm leading-6 text-slate-700">
          <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-rose-900">
            <p className="font-semibold">This permanently deletes the supervised family profile and all Guardian Mode activity tied to it.</p>
      ...
      >
        <div className="space-y-5 text-sm leading-6 text-slate-700">
          <p>
            Guardian Mode is intended for parent-supervised use. It is designed for situations where a parent or guardian is
          ...
          <section className="space-y-2">
            ...
            <ul className="list-disc space-y-2 pl-5">
              ...
              <li>Do not enable Guardian Mode for children or youth you do not supervise directly.</li>
          ...
          <section className="space-y-2">
            <h3 className="text-base font-semibold text-slate-950">What Guardian Mode is for</h3>
            <ul className="list-disc space-y-2 pl-5">
          ...
          <section className="space-y-2">
            ...
            <p>
              Guardian Mode does not replace direct parental oversight. Parents and guardians remain responsible for account security, device access,
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs leading-5 text-slate-500">
            This guidance will expand as Guardian Mode approval flows, remote device controls, and child-specific safeguards are added.
          </div>
